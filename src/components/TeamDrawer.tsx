...
export default function TeamDrawer(){
  ...
  return(
    <>
      ...
      <div style={{position:'fixed',top:0,right:0,bottom:0,
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(4,1fr)',borderBottom:'1px solid var(--border)',flexShrink:0}}>
          {[
            ...
            <div key={s.label} style={{padding:'8px 4px',textAlign:'center',borderRight:'1px solid var(--border)'}}>
              <div style={{fontSize:8,color:'var(--muted)',letterSpacing:.5}}>{s.label}</div>
              <div style={{fontSize:12,fontWeight:700,marginTop:2,color:s.color??'var(--text)'}}>{s.value}</div>
            </div>
          ))}
        </div>
            ...
            roles.map(role=>{
              ...
              return(
                <div key={role} style={{marginBottom:14}}>
                  <div style={{fontSize:10,letterSpacing:1,color:'var(--muted)',marginBottom:5,
                  ...
                  <div style={{display:'flex',flexDirection:'column',gap:5}}>
                    {rp.map(({e,p})=>p&&(
                      <div key={e.playerId} style={{display:'flex',alignItems:'center',gap:8,
                        padding:'6px 9px',background:'var(--surface2)',borderRadius:7,border:'1px solid var(--border)'}}>
                        <div style={{flex:1,minWidth:0}}>
                          ...
                          <div style={{display:'flex',gap:4,marginTop:2}}>
                            ...
                            <span style={{fontSize:10,color:'var(--muted)'}}>⭐{p.rating} · {p.form}</span>
                          </div>
                        </div>
      ...
      {showShare&&ts&&(
        <ShareCard team={teamInfo} players={players} purseLeft={ts.purse}
          ownerName={ts.ownerName??'AI'} onClose={()=>setShowShare(false)}/>
      )}
    </>
  );
}
