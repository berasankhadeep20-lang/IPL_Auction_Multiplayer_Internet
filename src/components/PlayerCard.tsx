...
import { generateAvatar } from '../utils/avatarCanvas';
...
function AvatarImg({ player, size, style }: { player: Player; size: number; style: React.CSSProperties }) {
  const [src, setSrc] = useState('');
  const generated = useRef(false);
  useEffect(() => {
    if (generated.current) return;
    generated.current = true;
    // Generate after paint so it doesn't block
    requestAnimationFrame(() => {
      setSrc(generateAvatar(player.name, player.role, size));
    });
  }, [player.id]);
  const color = ROLE_COLOR[player.role];
  if (!src) return (
    <div style={{ ...style, background:`linear-gradient(135deg,${color}22,#1a1e25)`,
      display:'flex', alignItems:'center', justifyContent:'center',
      color, fontSize: size * 0.26, fontWeight:700 }}>
      {player.name.split(' ').map(w=>w[0]).slice(0,2).join('')}
    </div>
  );
  return <img src={src} alt={player.name} style={style} />;
}
...
export default function PlayerCard({ player, large, price, flip }: Props) {
  ...
  if (!large) return (
    <div style={{ background:'var(--surface2)', border:'1px solid var(--border)',
      borderRadius:8, padding:'7px 10px', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
        <AvatarImg player={player} size={60}
          style={{ width:30, height:30, borderRadius:'50%', objectFit:'cover',
            border:`2px solid ${rc}44`, flexShrink:0 }}/>
        <div>
      ...
      {price !== undefined && (
        <span style={{ color:'var(--gold)', fontFamily:"'Barlow Condensed',sans-serif",
          fontSize:14, fontWeight:700, flexShrink:0 }}>{formatPrice(price)}</span>
      )}
    </div>
  );
    ...
    <div style={{ perspective:'1200px' }}>
      <div style={{
        ...
        <div style={{
          ...
          padding:'14px 16px', display:'flex', gap:14, alignItems:'stretch',
          boxShadow:`0 0 30px ${rc}18`,
        }}>
          {/* Avatar column */}
          <div style={{ flexShrink:0, display:'flex', flexDirection:'column', alignItems:'center', gap:5 }}>
            <div style={{ width:86, height:108, borderRadius:10, overflow:'hidden',
              border:`2px solid ${rc}66`, background:'var(--surface)',
              boxShadow:`0 4px 20px ${rc}33`, flexShrink:0 }}>
              <AvatarImg player={player} size={200}
                style={{ width:'100%', height:'100%', objectFit:'cover' }}/>
            </div>
            <div style={{ background:`${rc}22`, border:`1px solid ${rc}55`, color:rc,
          ...
          <div style={{ flex:1, minWidth:0 }}>
            <div style={{ display:'flex', justifyContent:'space-between',
              ...
              <h2 style={{ fontFamily:"'Rajdhani',sans-serif",
                fontSize:'clamp(16px,3vw,22px)', fontWeight:700, lineHeight:1.1 }}>
                {player.name}
              </h2>
            ...
            <div style={{ display:'flex', gap:5, marginTop:6, flexWrap:'wrap', alignItems:'center' }}>
              ...
              <span style={{ background:`${FORM_COLOR[player.form]}22`,
                border:`1px solid ${FORM_COLOR[player.form]}55`,
                color:FORM_COLOR[player.form],
        ...
        <div style={{
          ...
          <div style={{ fontSize:13, color:'rgba(245,200,66,0.35)',
            letterSpacing:3, fontFamily:"'Barlow Condensed',sans-serif" }}>
            IPL AUCTION 2025
          </div>
          <div style={{ fontSize:44, color:'rgba(245,200,66,0.12)',
            fontFamily:"'Bebas Neue',sans-serif", letterSpacing:4 }}>🏏</div>
          <div style={{ fontSize:10, color:'rgba(255,255,255,0.12)', letterSpacing:2 }}>NEXT PLAYER</div>
        </div>
      </div>
    </div>
  );
...
function S({ label, value }: { label:string; value:string|number }) {
  return (
    <div>
      ...
      <div style={{ fontSize:12, fontWeight:600, marginTop:1, color:'var(--text)' }}>{value}</div>
    </div>
  );
}
